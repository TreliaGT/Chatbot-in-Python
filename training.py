...
# Process each pattern and extract words and classes
for intent in intents['intents']:
    for pattern in intent['patterns']:
        wordList = nltk.word_tokenize(pattern) # Tokenize each word in the pattern
        words.extend(wordList)
        documents.append((wordList , intent['tag']))
        if intent['tag'] not in classes:
            classes.append(intent['tag'])

# Lemmatize and filter out punctuation from the words list
...
pickle.dump(words , open('words.pkle' , 'wb'))
pickle.dump(classes, open('classes.pkle' , 'wb'))
